...
const evm_wallet_service_1 = require("./evm-wallet.service");
const evm_config_1 = require("../config/evm.config");
class PaymentService {
    async processPayment(cardUuid, amount, merchantId, chainKey, metadata) {
        try {
            // 1. Validate card
            const card = await this.validateCard(cardUuid);
            // 2. Get user
            const user = await User_model_1.User.findById(card.userId).select('+evmEncryptedPrivateKey');
            if (!user)
            ...
            const chain = (0, evm_config_1.getEVMChain)(chainKey || evm_config_1.DEFAULT_EVM_CHAIN);
            if (!chain)
            ...
            try {
                ...
                const txResult = await this.executeBlockchainTransaction(user, merchant.evmWalletAddress, amount, chain);
    ...
    async executeBlockchainTransaction(user, recipientAddress, amount, chainConfig) {
        // Decrypt private key
        ...
        logger_1.default.info('EVM transaction completed:', {
    ...
    async retryTransaction(transactionId, userId) {
        ...
        const newTransaction = await this.processPayment(originalTransaction.cardUuid, originalTransaction.amount, originalTransaction.merchantId.toString(), undefined, {
