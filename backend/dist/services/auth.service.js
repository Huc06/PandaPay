...
class AuthService {
    get redisClient() {
        return (0, redis_config_1.getRedisClient)();
    }
    async register(userData) {
        ...
        const evmWallet = evm_wallet_service_1.EVMWalletService.createWallet();
        const encryptedEvmPrivateKey = (0, encryption_service_1.encryptPrivateKey)(evmWallet.privateKey);
        ...
        logger_1.default.info(`Created EVM wallet for user ${user.email}: ${evmWallet.address}`);
        return user;
    }
    ...
    async sendOTP(phoneNumber) {
        ...
        // Store OTP in Redis
        await this.redisClient.setex(`otp:${phoneNumber}`, constants_1.CONSTANTS.OTP_EXPIRY_MINUTES * 60, otp);
        // Send OTP via SMS
